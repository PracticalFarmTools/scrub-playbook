/**
 * InstrumentInspector — Mobile-First Bottom-Sheet Inspector
 *
 * A touch-optimized bottom-sheet that replaces the centered modal.
 * Features:
 *   • Slide-up entrance from bottom of viewport
 *   • Swipe-down gesture to dismiss (80px threshold)
 *   • Drag handle pill for gesture affordance
 *   • Large full-width Close button for one-handed thumb reach
 *   • Backdrop tap + Escape key dismiss
 *
 * Content: AST category header, large instrument image,
 *          practical tip, verified-by footer.
 */
import { useEffect, useCallback, useRef, useState, memo } from 'react';
import { X, Lightbulb, ShieldCheck, Clock, ChevronDown } from 'lucide-react';
import InstrumentPhoto from '../tray/InstrumentPhoto';
...
function InstrumentInspector({ instrument, tray, onClose }) {
  ...
  // ── Dismiss via animation class ──
  const sheetClass = dismissing
    ? 'inspector-sheet inspector-sheet-dismiss'
    : 'inspector-sheet';

  return (
    <div
      className={`inspector-backdrop ${dismissing ? 'inspector-backdrop-dismiss' : ''}`}
      onClick={onClose}
    >
      <div
        ref={sheetRef}
        ...
        style={{
          transform: dragOffset > 0 && !dismissing
            ? `translateY(${dragOffset}px)`
            : undefined,
          transition: dragOffset > 0 && !dismissing
            ? 'none'
            : undefined,
        }}
      >
        {/* ── Drag Handle ── */}
        <div className="inspector-drag-handle-area">
        ...
        {/* ── LARGE CLOSE BUTTON (one-handed thumb reach) ── */}
        <button
          onClick={onClose}
...
export default memo(InstrumentInspector);
